...
// Test data
// NOTE: These accounts must exist in Supabase before running this suite.
const testUsers = {
...
// Wrap entire suite in serial mode so all tests share the same worker process
// and the module-level auditFindings array is preserved across tests.
// (fullyParallel: true in playwright.config.ts would otherwise scatter tests
// across workers, splitting the findings array.)
test.describe('Comprehensive QA Audit', () => {
  test.describe.configure({ mode: 'serial' });

  // Register console error listener BEFORE each test (not after)
  ...
  test.describe('Homepage & Marketing Pages', () => {
    ...
    test('About page - Named CTA elements work', async ({ page }) => {
      await page.goto(`${BASE_URL}/en/about`);

      // Scope to named CTA buttons only — clicking all a[href] would follow
      // external links and downloads, destabilising the test.
      const ctaTexts = ['Contact', 'Learn More', 'Get Started'];
      for (const text of ctaTexts) {
        try {
          ...
          if (await button.isVisible()) {
            ...
            console.log(`✅ About page - "${buttonText}" button works`);
            await page.goto(`${BASE_URL}/en/about`);
          }
        } catch (error) {
          addFinding({
            page: 'About Page',
            element: `"${text}" CTA button`,
            issue: 'CTA button is broken or non-responsive',
            severity: 'medium',
            expected: 'Button should perform action or navigate',
            actual: 'Button failed to respond',
            reproduction: `Click on "${text}" button on About page`
          });
        }
      }
    });
    ...
    test('Contact page - Form submission works', async ({ page }) => {
      ...
      try {
        ...
        if (await nameInput.isVisible()) {
          ...
          // Use .or() chaining — comma in a single locator string is CSS selector
          // syntax, not "OR" for text matching
  ...
  test.describe('Authentication Flows', () => {
    ...
    test('Forgot Password - Flow works correctly', async ({ page }) => {
      ...
      try {
        ...
        const successMessage = page.locator('text=email sent')
          .or(page.locator('text=check your email'))
          .or(page.locator('text=link sent'));
        ...
        if (await createButton.isVisible()) {
          ...
          if (await formTitle.isVisible()) {
            ...
            if (await submitButton.isVisible()) {
              ...
              const successMessage = page.locator('text=success').or(page.locator('text=created'));
  ...
  test.describe('Student Portal', () => {
    ...
    test('Student Courses - Course enrollment and viewing work', async ({ page }) => {
      ...
      try {
        // Fix: await .all() once, not twice
        const courseCards = await page.locator('.course-card, [data-testid="course-card"], .card').all();
  ...
  test.describe('Parent Portal', () => {
    test.beforeEach(async ({ page }) => {
      // Parents use the student login route (no dedicated /login/parent route)
      await page.goto(`${BASE_URL}/en/login/student`);
    ...
    test('Parent Invoices - Invoice viewing works', async ({ page }) => {
      ...
      try {
        // Fix: await .all() once, not twice
        const invoiceCards = await page.locator('.invoice-card, [data-testid="invoice"], .card').all();
  ...
  test.describe('VR Features', () => {
    test('VR Eduverse - VR navigation works', async ({ page }) => {
      ...
      try {
        const navButtons = await page.locator('button:has-text("Field Trips"), button:has-text("Science"), button:has-text("Navigate")').all();
  ...
  test.describe('E-commerce Features', () => {
    test('Cart - Add to cart and checkout work', async ({ page }) => {
      ...
      try {
        // Fix: await .all() once, not twice
        const addToCartButtons = await page.locator('button:has-text("Add to Cart"), button:has-text("Enroll"), button:has-text("Buy")').all();
    ...
    test('Checkout - Checkout form works', async ({ page }) => {
      ...
      try {
        ...
        if (formFields.length > 0) {
          console.log(`✅ Checkout - Found ${formFields.length} form field(s)`);
    ...
    test('Search - Search functionality works', async ({ page }) => {
      ...
      try {
        ...
        if (await searchInput.isVisible()) {
          ...
          // Fix: await .all() once
          const results = await page.locator('.search-result, .course-card, .card').all();
          if (results.length > 0) {
  ...
  test.afterAll(async () => {
    ...
    const reportPath = path.join(reportDir, 'qa-audit-report.json');

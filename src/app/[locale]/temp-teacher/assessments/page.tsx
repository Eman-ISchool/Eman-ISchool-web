    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                ...
                <Link
                    href={withLocalePrefix('/temp-teacher/assessments/new', locale)}
            ...
            {activeTab === 'manage' ? (
                manageAssessments.length === 0 ? (
                    <EmptyState
                        ...
                        action={{ label: isArabic ? 'إنشاء اختبار جديد' : 'Build New Test', href: withLocalePrefix('/temp-teacher/assessments/new', locale) }}
                    />
                ) : (
                    <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
                        {manageAssessments.map((assessment: any) => (
                            <Link
                                key={assessment.id}
                                href={withLocalePrefix(`/temp-teacher/assessments/${assessment.id}`, locale)}
                ...
                resultAssessments.length === 0 ? (
                    ...
                    <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
                        {resultAssessments.map((assessment: any) => (
                            <Link
                                key={assessment.id}
                                href={withLocalePrefix(`/temp-teacher/assessments/${assessment.id}/results`, locale)}
                                className="group bg-white rounded-2xl border border-gray-100 p-5 hover:shadow-lg transition-all duration-200"
                            >
                                <h3 className="font-semibold text-gray-900 group-hover:text-[var(--color-primary)] transition-colors">
                                    {assessment.title}

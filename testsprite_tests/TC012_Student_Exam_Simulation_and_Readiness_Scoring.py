...
async def run_test():
    ...
    try:
        ...
        await page.goto('http://localhost:3000/ar/student/assessments', wait_until="commit", timeout=30000)
        try:
            await page.wait_for_load_state("domcontentloaded", timeout=5000)
        except async_api.Error:
            pass
        ...
        # Assert assessments page is accessible (student session persists)
        current_url = page.url
        assert '/student' in current_url or '/assessments' in current_url, (
            f"Student was unable to access assessments. Current URL: {current_url}"
        )
        await asyncio.sleep(2)
